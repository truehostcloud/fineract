...
public class SurveyAnalyticsServiceImpl implements SurveyAnalyticsService {
    ...
    public SurveyResponseAnalyticsData getSurveyResponseAnalytics(String surveyName, LocalDate startDate, LocalDate endDate) {
        ...
        final SurveyResponseAnalyticsData analytics = new SurveyResponseAnalyticsData().setSurveyName(surveyName).setStartDate(startDate)
                .setEndDate(endDate);

        try {

            MapSqlParameterSource surveyParams = new MapSqlParameterSource("surveyName", surveyName);
            List<Map<String, Object>> surveyAndQuestions = namedParameterJdbcTemplate.queryForList(SURVEY_AND_QUESTIONS_QUERY,
                    surveyParams);
            ...
            MapSqlParameterSource totalParams = new MapSqlParameterSource().addValue("surveyId", surveyId)
            ...
            List<String> questionIds = surveyAndQuestions.stream().map(row -> String.valueOf(row.get("question_id")))
                    .collect(Collectors.toList());

            MapSqlParameterSource responsesParams = new MapSqlParameterSource("questionIds", questionIds);
            List<Map<String, Object>> responses = namedParameterJdbcTemplate.queryForList(RESPONSES_QUERY, responsesParams);
            ...
            MapSqlParameterSource statsParams = new MapSqlParameterSource().addValue("surveyId", surveyId)
                    .addValue("startDate", startDate != null ? Date.valueOf(startDate) : null)
                    .addValue("endDate", endDate != null ? Date.valueOf(endDate) : null);

            List<Map<String, Object>> responseStats = namedParameterJdbcTemplate.queryForList(RESPONSE_STATS_QUERY, statsParams);

            List<Map<String, Object>> nullResponses = namedParameterJdbcTemplate.queryForList(NULL_RESPONSES_QUERY, statsParams);
            ...
            for (Map<String, Object> stat : responseStats) {
                ...
                statsByQuestionAndResponse.computeIfAbsent(qId, k -> new HashMap<>()).put(rId, stat);
            }

            for (Map<String, Object> nullStat : nullResponses) {
                String qId = String.valueOf(nullStat.get("question_id"));

                statsByQuestionAndResponse.computeIfAbsent(qId, k -> new HashMap<>()).put("null", nullStat);
            }
            ...
            for (Map<String, Object> question : surveyAndQuestions) {
                ...
                SurveyResponseAnalyticsData.QuestionAnalytics qAnalytics = new SurveyResponseAnalyticsData.QuestionAnalytics()
                        .setQuestionCode(questionKey).setQuestionText(questionText);
                ...
                Map<String, Map<String, Object>> questionStats = statsByQuestionAndResponse.getOrDefault(questionId,
                ...
                for (Map<String, Object> response : questionResponses) {
                    ...
                    choices.add(new SurveyResponseAnalyticsData.QuestionAnalytics.ChoiceAnalytics().setChoiceText(responseText)
                            .setCount(count).setPercentage(percentage));
                }
                ...
                if (nullStats != null) {
                    ...
                    if (nullStats.containsKey("percentage")) {
                        ...
                        int totalForQuestion = questionResponses.stream().mapToInt(r -> {
                    ...
                    choices.add(new SurveyResponseAnalyticsData.QuestionAnalytics.ChoiceAnalytics().setChoiceText("NULL")
                            .setCount(nullCount).setPercentage(nullPercentage));
                }
